...
{
	public static class Z80DisassemblerTable
	{
		[Flags]
		...
		public class OpCode
		{
			...
			public OpCode(string mnemonic, uint t_states, uint t_states2, OpCodeFlags flags = OpCodeFlags.Continues)
			{
				Mnemonic = mnemonic;
				TStates = t_states;
				TStates2 = t_states2;
				...
				if (mnemonic != null)
				{
					...
					if (mnemonic.StartsWith("IN ") || mnemonic.StartsWith("OUT "))
						Flags |= OpCodeFlags.PortRef;

					if (mnemonic.StartsWith("CALL "))
						Flags |= OpCodeFlags.Call;

					if ((Flags & OpCodeFlags.ImplicitA) != 0)
					{
